...
- **面板形式(形式乙,已定)**:主菜单保持扁平 7 项;BBR / SSH / System 在**执行前先打印一次**对应面板;SS / Mihomo / Reality 在各自**子菜单**顶部显示面板。
...
- **运行前检测**:已安装则输出现有配置 + `vless://` 分享链接(**不含二维码**),再询问是否重装(回车默认**否**=保留并退出;`y` 才继续);非交互(带参数/环境变量)自动继续。
